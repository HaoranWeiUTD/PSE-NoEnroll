...
class SqueezedGRU(nn.Module):
    ...
    def forward(self, input: Tensor, aemb=None, h=None) -> Tuple[Tensor, Tensor]:
        input = self.linear_in(input)
        if aemb is not None:
            input = self.multihead_attn(input, aemb, aemb, is_causal=True)[0] * input
...
def get_erb(input, erb_widths, sr: int=48000):

    erb = erb_fb(erb_widths, sr)
