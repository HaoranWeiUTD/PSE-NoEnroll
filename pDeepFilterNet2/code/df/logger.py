...
def log_model_summary(model: torch.nn.Module, verbose=False):
    ...
    # aux_eca = torch.randn([b, 1, 1, 192]).to(device)
    aux_erb = torch.randn([b, 1, 1000, 32]).to(device)
    aux_spec = torch.randn([b, 1, 1000, 96, 2]).to(device)

    warnings.filterwarnings("ignore", "RNN module weights", category=UserWarning, module="torch")
    macs, params = ptflops.get_model_complexity_info(
        deepcopy(model),
        (t,),
        input_constructor=lambda _: {"spec": spec, "feat_erb": feat_erb, "feat_spec": feat_spec, "aux_erb": aux_erb, "aux_spec": aux_spec},
        as_strings=False,
