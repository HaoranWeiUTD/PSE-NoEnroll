...
from .audio_ecapa import WaveReader, EcapaReader
...
def make_dataloader(is_train,
    ...
    dataset = My_dataset(mix_dir, ref_dir, aux_wav_dir, sample_rate=df_state.sr())
...
class My_dataset(Dataset):
    ...
    """
    def __init__(self, mix_scp, ref_scp, aux_wav_scp, sample_rate):
        self.mix = WaveReader(mix_scp, sample_rate=sample_rate)
        self.ref = WaveReader(ref_scp, sample_rate=sample_rate)
        self.aux_wav = WaveReader(aux_wav_scp, sample_rate=sample_rate)
    ...
    def __getitem__(self, index):
        ...
        mix = self.mix[key]
        ref = self.ref[key]
        aux_wav = self.aux_wav[key]

        return {
            "mix": mix.astype(np.float32),
            "ref": ref.astype(np.float32),
            "aux_wav": aux_wav.astype(np.float32),
            "aux_len": aux_wav.shape[-1],
			"key": key
        }
    def __len__(self):
        return len(self.mix)
...
class My_dataLoader(object):
    ...
    def _split_aux(self, chunk_list, aux_max_len):
        # lens_list = []
        # for chunk_item in chunk_list:
        #     lens_list.append(chunk_item['aux_len'])
        # min_len = np.min(lens_list)
        for idx in range(len(chunk_list)):
            # total_len = len(chunk_list[idx]['aux'])
            # a = random.randint(0, min_len - aux_max_len) if self.train else 0
            a = 0
            chunk_list[idx]["aux_wav"] = chunk_list[idx]["aux_wav"][a:a+aux_max_len] 
    ...
    def _merge(self, chunk_list):
        ...
        for s in range(0, N - self.batch_size + 1, self.batch_size):
            ...
            batch = default_collate(self._split_aux(chunk_list[s:s + self.batch_size], self.aux_max_len))
            ...
            # (B, F, T): (2, 481, 15000)
            batch['enrol'], batch['aux_erb'], batch['aux_spec']  = self.df_features(batch["aux_wav"], self.df_state, self.nb_df)
            #batch['enrol_eca'] = batch["aux_eca"]

            del batch['mix'], batch['ref'], batch['aux_wav'], batch['enrol']
            blist.append(batch)
