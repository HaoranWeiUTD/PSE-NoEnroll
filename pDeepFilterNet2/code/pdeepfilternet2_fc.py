...
class SpearEncoder(nn.Module):
    def __init__(self) -> None:
        ...
        self.linear_in = nn.Sequential(
            GroupedLinearEinsum(self.emb_in_dim, self.emb_out_dim, p.gru_groups), nn.ReLU()
        )
    
    def forward(self, aux_erb, aux_spec):
        ae0 = self.erb_conv0(aux_erb)  # [B, 64, 300, 32]
        ae1 = self.erb_conv1(ae0)  # [B, 64, 300, 16]
        ae2 = self.erb_conv2(ae1)  # [B, 64, 300, 8]
        ae3 = self.erb_conv3(ae2)  # [B, 64, 300, 8]
        ac0 = self.df_conv0(aux_spec)  # [B, 64, 300, 96]
        ac1 = self.df_conv1(ac0)  # [B, 64, 300, 48]
        
        cemb = ac1.permute(0, 2, 3, 1).flatten(2)  # [B, 300, 64*48=3072]
        cemb = self.df_fc_emb(cemb)  # [B, 300, 512]
        emb = ae3.permute(0, 2, 3, 1).flatten(2)  # [B, 300, 512]
        emb = self.combine(emb, cemb)   #[B, 300, 1024]
        emb = self.linear_in(emb)  # [B, T, 256]
        return torch.mean(emb, 1, keepdim=True)
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.tfe = nn.TransformerEncoderLayer(d_model=256, dim_feedforward=512, nhead=8, batch_first=True)

    def forward(self, feat_erb: Tensor, feat_spec: Tensor, aemb: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        e0 = self.erb_conv0(feat_erb)  # [B, 64, 300, 32]
        e1 = self.erb_conv1(e0)  # [B, 64, 300, 16]
        e2 = self.erb_conv2(e1)  # [B, 64, 300, 8]
        e3 = self.erb_conv3(e2)  # [B, 64, 300, 8]
        c0 = self.df_conv0(feat_spec)  # [B, 64, 300, 96]
        c1 = self.df_conv1(c0)  # [B, 64, 300, 48]
        
        cemb = c1.permute(0, 2, 3, 1).flatten(2)  # [B, 300, 64*48=3072]
        cemb = self.df_fc_emb(cemb)  # [B, 300, 512]

        emb = e3.permute(0, 2, 3, 1).flatten(2)  # [B, 300, 512]
        emb = self.combine(emb, cemb)   #[B, 300, 1024]
        emb, _ = self.emb_gru(emb, aemb=aemb)  # [B, T, 256]

        emb = self.tfe(emb, is_causal=True)
        #emb = self.tfe(emb)
...
class DfNet(nn.Module):
    ...
    def __init__(
        ...
        self.enc = Encoder()
        self.enc_aux = SpearEncoder()
        self.speaker_fc = SpeakerEnc_Fc(256, 256)
    ...
    def forward(
        ...
        feat_spec = feat_spec.squeeze(1).permute(0, 3, 1, 2)   # feat_spec: (B, 2, 300, 96)
        feat_erb = self.pad(feat_erb)    # feat_erb: (B, 1, 300, 32)
        feat_spec = self.pad(feat_spec)

        aux_spec = aux_spec.squeeze(1).permute(0, 3, 1, 2)   # feat_spec: (B, 2, 300, 96)
        aemb = self.enc_aux (aux_erb, aux_spec)

        aemb = self.speaker_fc(aemb)

        e0, e1, e2, e3, emb, _ = self.enc(feat_erb, feat_spec, aemb)    
        m = self.erb_dec(emb, e3, e2, e1, e0)

        spec = self.mask(spec, m)
        df_coefs, df_alpha = self.df_dec(emb)
        ...
        return spec, m, df_alpha
if __name__ == "__main__":
    Dfnet = DfNet()
    input = torch.randn(16, 1, 300, 481)
    feat_erb = torch.randn(16, 1, 300, 32)
    feat_spec = torch.randn(16, 1, 300, 96)
    enh, _, lsnr, _ = Dfnet.forward(
                spec=input,
                feat_erb=feat_erb,
                feat_spec=feat_spec,
            )
