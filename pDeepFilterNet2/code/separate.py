...
class My_dataset(Dataset):
    ...
    """
    def __init__(self, mix_scp, aux_scp, sample_rate):
...
def main(args, dtype=torch.int16):
    ...
    ds = My_dataset(mix_scp=args.mix_scp, aux_scp=args.aux_scp, sample_rate=df_sr)

    start = time.time()
    logger.info("start separate audio")
    for i, data_dict in enumerate(ds):
        noisy_td = torch.as_tensor(data_dict["mix"]).unsqueeze(0)
        aux = torch.as_tensor(data_dict["aux"]).unsqueeze(0)
        key = data_dict["key"]
        
        enh = enhance(model, df_state, noisy_td, aux)[0]
...
def enhance(model: nn.Module, df_state: DF, audio: Tensor, aux: Tensor):
    ...
    _, aux_erb_feat, aux_spec_feat = df_features(aux, df_state, nb_df, device=get_device())
...
def run():
    ...
    parser.add_argument(
        "--aux_scp",
        type=str,
        default="/Share/wsl/data/DC-data/DNS5_track2_bt_16k/path/aux_wav.scp",
    )
    parser.add_argument(
        "--save_path",
        type=str,
