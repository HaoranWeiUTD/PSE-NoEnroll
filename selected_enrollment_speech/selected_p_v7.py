...
class ComputeScore:
    ...
    def __call__(self, audio, sampling_rate=16000):
        ...
        snr = self.wada_snr(audio)
    
        return snr * ratio_of_speech, snr, ratio_of_speech

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--wav_path", default="/Share/wsl/data/win/youtube_data_lecture/path/noisy_path.txt")
    parser.add_argument("--save_dir", default="/Share/wsl/data/win/youtube_data_lecture/enrol_select-5s-snr")
    parser.add_argument("--log_path", default="/Share/wsl/data/win/youtube_data_lecture/log-5s_v1.txt")
    ...
    with open(args.log_path, "w") as log, open(args.wav_path, 'r') as f:
        for path in f:
            ...
            record = {"snr_score":0, "snr":0, "speech_ratio":0, "audio_seg":None}
            for start_time in range(0, len(audio), fs):
                audio_seg = audio[start_time:start_time+CHUNK_SIZE]
                if len(audio_seg) < CHUNK_SIZE:
                    continue

                snr_score, snr, speech_ratio = computescore(audio_seg)
                if snr_score > record['snr_score']:
                    record['snr_score'] = snr_score
                    record['snr'] = snr
            ...
            log.write(f"{path}\t{record['snr_score']}\t{record['snr']}\t{record['speech_ratio']}\n")
